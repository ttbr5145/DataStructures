...
namespace List{
    ...
    class sllist                                  //单向链表，左闭右开，最末项为(0，NULL)无效项
    {
        public:
            ...
            void ins_back(Data value)             //插入值，顶层
            {
                insert(head,value);
                length++;
            }
        ...
        private:
            ...
            void insert(Node* now, Data value){   //底层
                if(now->next != NULL)
    ...
    class dllist
    {
        ...
        private:
            void insert_back(Node* now, Data value)
            {
                if (now->next != tail)
                    insert_back(now->next, value);
                else
                {
